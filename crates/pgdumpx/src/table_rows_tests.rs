use crate::{Archive, FieldRef, OwnedField};
...
#[test]
fn find_first_preserves_parser_consumed_byte_accounting_and_stops_at_match() {
    let first_physical_row = b"1\tEARLY-100\tcustomer-a\tplain\t\n";
    ...
    for fixture_name in ["pg18-none-copy-basic.dump", "pg18-gzip-copy-basic.dump"] {
        ...
        assert_eq!(
            rows.consumed_input_bytes(),
            u64::try_from(first_physical_row.len()).unwrap(),
            "decoder or BufRead read-ahead must not affect find_first accounting for {fixture_name}"
        );
    }
}

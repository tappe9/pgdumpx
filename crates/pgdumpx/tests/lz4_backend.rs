...
#[cfg(feature = "lz4")]
#[test]
fn official_lz4_fixture_streams_rows_and_stops_find_first_early() {
    let bytes = fixture();
    let mut archive = Archive::open(Cursor::new(bytes.clone())).unwrap();
    assert!(matches!(archive.header().compression(), Compression::Lz4));
    let id = archive
...
#[cfg(feature = "lz4")]
#[test]
fn lz4_handles_one_byte_source_reads_and_one_byte_custom_chunks() {
    let fixture = fixture();
    ...
    let bytes = archive_with_block(LZ4_COMPRESSION, &block);
    let mut archive = Archive::open(Cursor::new(bytes)).unwrap();
...
#[cfg(feature = "lz4")]
#[test]
fn corrupt_and_truncated_lz4_are_typed_decompression_errors() {
    let compressed = compressed_entry_bytes(&fixture());
...
#[cfg(feature = "lz4")]
#[test]
fn lz4_raw_limits_distinguish_exact_eof_from_limit_exceeded() {
...
#[cfg(not(feature = "lz4"))]
#[test]
fn lz4_metadata_opens_but_selected_read_reports_backend_unavailable() {
...
fn fixture() -> Vec<u8> {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/fixtures/archives/pg18-lz4-copy-basic.dump");
    std::fs::read(path).expect("official LZ4 fixture must be readable")
}
